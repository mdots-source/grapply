...
const defaultJokes = [
  ...
  "Шутка про гривну",
  "Шутка про рубль",
  "История, которую нельзя рассказывать при родителях",
  "Фраза, после которой все поняли: это точно Марио",
  "Легендарный момент на кухне",
  "Шутка, которая смешнее с каждым годом",
  "Главная birthday-легенда вечера",
];

const jokesKey = "mario-birthday-jokes-v1";
const gameKey = "mario-birthday-game-v1";
...
export function MarioBirthdayQuiz() {
  ...
  return (
    <main className="min-h-screen bg-[#08080c] text-[#f7f4ec]">
      <div className="mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-8">
        <section className="overflow-hidden rounded-[28px] border border-white/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.035))] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)] sm:p-8">
          <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-[#ffd166]">Birthday edition</p>
          <h1 className="max-w-3xl text-[42px] font-black leading-[0.92] tracking-[-0.05em] sm:text-[78px]">Mario Joke Battle</h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-[#b6ad9c] sm:text-lg">
            Выбирай, какая шутка смешнее. Победитель идёт дальше, пока не найдём главную легенду вечера.
          </p>
        </section>

        <section className="mt-4 rounded-[28px] border border-white/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.08),rgba(255,255,255,0.035))] p-4 shadow-[0_24px_80px_rgba(0,0,0,0.35)] sm:p-5">
          ...
          {finished ? (
            <div className="mt-5 rounded-[24px] border border-[#61e8b4]/35 bg-[#61e8b4]/10 p-5 sm:p-6">
              <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-[#ffd166]">Winner</p>
              <h2 className="text-4xl font-black leading-none tracking-[-0.04em] sm:text-6xl">{game.champion}</h2>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-[1fr_auto_1fr]">
              <ChoiceCard kicker="Вариант A" text={game.champion} onClick={() => choose("left")} />
              <div className="self-center justify-self-center text-xs font-black uppercase text-[#ff3d6e]">vs</div>
              <ChoiceCard kicker="Вариант B" text={challenger ?? ""} onClick={() => choose("right")} />
            </div>
          )}
...
function ChoiceCard({ kicker, text, onClick }: { kicker: string; text: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="min-h-[150px] rounded-[24px] border border-white/15 bg-[#14141d] p-5 text-left transition hover:-translate-y-0.5 hover:border-[#ffd166]/60 hover:bg-[#1d1d29] sm:min-h-[190px] sm:p-6"
    >
      <span className="mb-4 block text-xs font-black uppercase tracking-[0.14em] text-[#ffd166]">{kicker}</span>
      <span className="block text-2xl font-black leading-tight tracking-[-0.03em] sm:text-4xl">{text}</span>
    </button>
  );
}
