...
import { passwordResetEmailBody, queueEmail } from "@/lib/email/outbox";
import { getRequestUrl } from "@/lib/request-origin";
import { generateAuthActionLink, sendPasswordRecoveryEmail } from "@/lib/supabase/auth";
  ...
  try {
    if (isEmailDeliveryConfigured()) {
      ...
      const queued = await queueEmail({
        ...
        subject: "Reset your Grapply password",
        body: passwordResetEmailBody({ destinationUrl: actionLink }),
        metadata: { returnTo, inviteToken: inviteToken || undefined, delivery: "resend" },
      });
      if (!queued) throw new Error("Password reset email could not be queued.");
    } else {
      await sendPasswordRecoveryEmail(email, callbackUrl.toString());
    }
    return noStoreJson({ ok: true });
  } catch (error) {
    if (isAccountEnumerationSafeNoop(error)) return noStoreJson({ ok: true });
    return authFailureJson(error, getAuthEmailSendError(error, "Password reset email failed."), 400);
  }
}
...
function authFailureJson(error: unknown, fallback: string, status = 400) {
  ...
  recordAuthFailure({ requestId, message, status, action: "password-reset" });
